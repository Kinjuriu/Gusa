...
| D-003 | **Voice provider.** | In: Android `speech_to_text` (streaming, free, low latency). Out: ElevenLabs voice when online via the Worker, `flutter_tts` offline; the caller never knows which. Generic replies pre-synthesised and cached on the phone. | Ian | — | **DECIDED 2026-09-02** (amended the same day: ElevenLabs back for voice out) |
...
| D-013 | **Flutter only, no Kotlin.** | Native behaviour comes from plugins: `speech_to_text` 7.4, `flutter_tts` 4.2, `vibration` 3.2, `just_audio`, `shared_preferences`, `installed_apps` 2.1, `android_intent_plus` 5.2. `android/` holds manifest and Gradle config only. If a plugin gap ever forces native code, fork the plugin in its own repo; this repo stays Dart. Known for Phase 2: `flutter_accessibility_service` 1.2 exposes the event node, not the full tree, so screen reading will need native work later. | Ian | — | **DECIDED 2026-09-02** |
...
| D-015 | **Cloud key holder.** Who creates the Anthropic and ElevenLabs keys (OpenAI optional), sets monthly caps, and holds the Worker secrets? | Ian. Developers never receive a key. | Ian | Week 0 (W0.6) | OPEN |
| D-016 | **LLM provider for reply suggestions.** | Claude, model `claude-opus-5`, structured output, effort low, via `@anthropic-ai/sdk` in the Worker. OpenAI selectable with one Worker env var (`AI_PROVIDER`). V1.5 measures p95 latency against the 3 s budget; model choice stays Ian's. | Ian | — | **DECIDED 2026-09-02** |
...
- 2026-09-02 — ElevenLabs back in for voice out (D-003 amended). Claude is the default provider for reply suggestions (D-016). D-015 widened to all cloud keys. Plan review fixes applied: checkpoint timing, Week 0 dependencies, gesture scoping, numeric bars on T2.1 and I2.1, plugin list completed.
